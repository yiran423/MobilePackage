...
CLEAN() {
	...
	find "$PROJDIR/src/main/assets" -name "index.android.bundle" -exec rm {} \;
	find "$PROJDIR/src/main/assets" -name "index.android.bundle.meta" -exec rm {} \;
		...
		if [[ "$channelName" == "test" ]]; then
			...
			INFO "打包开始："	
		 	...
		 	if [[ $buildType = 'Debug' ]]; then
		 		...
		 		/usr/local/bin/react-native bundle --entry-file "index.android.js" --platform "android" --dev "true" --bundle-output "./android/app/src/main/assets/index.android.bundle" --assets-dest "./android/app/src/main/res/"
		 	fi
			gradle -PAPPLICATION_ID="$appPkg" -PAPP_NAME="$appName" -PAPP_VERSION_NAME="$appVersion" -PAPP_VERSION_CODE="$appVersionCode" -PBUILD_SCRIPT="${PROJDIR}/build.gradle" \
